...
{
    public interface ICAPBusiness
    {
        /// <summary>
        ///  Add thong tin vi pham cho cac khu vuc duoc danh gia
        /// </summary>
    ...
    public class CAPBusiness : BaseBusiness, ICAPBusiness
    {
        private Lazy<ICAPViolationAcess> violationAccess;
        public CAPBusiness(string appid, int uid) : base(appid, uid)
        {
            violationAccess =
                new Lazy<ICAPViolationAcess>(() => new CAPViolationAccess(appid, uid));
        }
        /// <summary>
        ...
        {
            return violationAccess.Value.InsertUpdateViolationToMultiZones(CAPInfo);
        }
        /// <summary>
        /// Lay danh sach vi pham cho theo dieu kien filter
        /// </summary>
        ...
        {
            return violationAccess.Value.FindZoneViolations(siteId, evaluationCalendarId, groupCateId, cateId,violationId,keyWord,page, pageSize, sourceClientId, startDate,  endDate,isSearchDate);
        }

        public CUDReturnMessage DeleteViolation(int violationId)
        {
            return violationAccess.Value.DeleteViolation(violationId);
        }
        /// <summary>
        ///  lay thong tin violationZone
        /// </summary>
        /// <param name="ViolationId"></param>
        /// <returns></returns>
        public SiteZoneViolationContract GetViolation(int ViolationId)
        {
            var data= violationAccess.Value.GetViolation(ViolationId);
            if (data == null) return null;
            return new SiteZoneViolationContract
            {
                ViolationId = data.ViolationId,
                SiteZoneId = data.EvaluationSiteZoneId,
                UriImageViolationError = data.UriImageViolationError,
                ViolationError = data.ViolationError,
                EvaluationDeptId = data.EvaluationDeptid,
                Propose = data.Propose,
                NoteReport = data.NoteReport,
                ViolationStatusId = data.ViolationStatusId,
                EvaluationSiteZoneId = data.EvaluationSiteZoneId,
                ReportDateEnd =data.ReportDateEnd??DateTime.Now.AddDays(30),
                ReportDeptId =data.ReportDeptId??0,
                ImageReportUri = data.ImageReportUri

            };
        }
        /// <summary>
        /// cap nhat giai trinh khac phuc loi vi pham
        /// </summary>
        /// <param name="model"></param>
        /// <returns></returns>
        public CUDReturnMessage UpdateZoneViolationReport(SiteZoneViolationContract model)
        {
            return violationAccess.Value.UpdateZoneViolationReport(model);
        }
    }
}

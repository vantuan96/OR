    ...
    public class JobScheduler
    {
        public static void Start()
        {
            ...
            ITrigger sync_oh_service_trigger = TriggerBuilder.Create()
                .WithCronSchedule(ConfigUrl.CF_SyncOHService_CS)
                //.WithCronSchedule("0 50 11 ? * *")
                .Build();
            ...
            //Auto cập nhật trạng thái ca mổ của khách hàng
            #region Tự động cập nhật trạng thái gói dịch vụ
            IJobDetail sync_auto_update_Status_job = JobBuilder.Create<UpdateORProcessJob>().Build();
            ITrigger sync_auto_update_trigger = TriggerBuilder.Create()
                .WithCronSchedule(ConfigUrl.CF_AutoUpdateStatus_CS)
                //.WithCronSchedule("0 50 11 ? * *")
                .Build();
            scheduler.ScheduleJob(sync_auto_update_Status_job, sync_auto_update_trigger);
            CustomLog.Instant.IntervalJobLog("AutoUpdateStatus job was created", Constant.Log_Type_Info, printConsole: true);
            #endregion .Tự động cập nhật trạng thái gói a mổ
        }
    }
}

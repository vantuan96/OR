...
namespace DataAccess
{
    public class BaseDataAccess
    {
        ...
        protected Entities DbContext { get; set; }
        ...
        protected string defaultLanguageCode
        {
            get
            {
                ...
                {
                    var defaultLang = "vi";
                    _defaultLanguageCode = defaultLang;
                }

                return _defaultLanguageCode;
            }
        }
        protected MemberExtendContract MemberExtInfo {
            get {
                return (MemberExtendContract)System.Web.HttpContext.Current.Session["adr.User"];
            }
        }

        public BaseDataAccess(string appid, int uid, string connStr = "ConnString.WebPortal")
        {
            this.appid = appid;
            this.uid = uid;
            
            //string connection = ConfigurationManager.ConnectionStrings[connStr].ConnectionString;
            this.DbContext = new Entities(connStr);


            string connectionString = "ConnString.PatientQueue";
            if (ConfigurationManager.ConnectionStrings[connectionString] != null)
            {
                _db = new DatabaseProviderFactory().Create(connectionString);
            }            
        }        
        protected IQueryable<T> PagedResult<T, TResult>(IQueryable<T> query, int pageNum, int pageSize,
                Expression<Func<T, TResult>> orderByProperty, bool isAscendingOrder, out int rowsCount)
        {
            /* set mặc định số record cần lấy khi truyền pageSize <= 0*/
            ...
            //Nếu trang cần lấy <= 0 hoặc tổng số record <= pageSize thì set măc định pageNum = 1
